 ...
 */
static int circuit_to_text (char * buf, struct circuit * c, size_t node, int is_right);
...
static void tree_to_circuit (struct tree * t, struct circuit * c, size_t node);
  ...
  while (keep_going)
    {
	  ...
	  if ((surface = TTF_RenderUTF8_Blended (font_equation, buf, color_white)))
            {
	  ...
	  switch (event.type)
	    ...
	    case SDL_MOUSEMOTION:
	      do 
	        {
		  int y = Y (cursor);
		  int x = (WORLDX (event.motion.x, event.motion.y, SPS * y) - SPS / 2) / SPS;
		  int z = (WORLDZ (event.motion.x, event.motion.y, SPS * y) + SPS / 2) / SPS;

		       if (x < 0)	{ x = 0; }
		  else if (x >= SIZE)	{ x = SIZE - 1; }
		       if (z < 0)	{ z = 0; }
		  else if (z >= SIZE)	{ z = SIZE - 1; }
	    ...
	    case SDL_MOUSEBUTTONDOWN:
	      switch (event.button.button)
	        {
		case SDL_BUTTON_LEFT:		object = GROUND; break;
		case SDL_BUTTON_WHEELDOWN:	if (Y (cursor) > MIN) cursor -= SIZE_2; break;
		case SDL_BUTTON_WHEELUP:	if (Y (cursor) < MAX) cursor += SIZE_2; break;
		default: break;
		}
	      break;
	    ...
	    case SDL_MOUSEBUTTONUP:
	      switch (event.button.button)
	        {
		case SDL_BUTTON_LEFT:		object = -1; break;
		default: break;
		}
	      break;
		...
		{
		case SDLK_g:		if (object == GROUND) object = -1; break;
		case SDLK_c:		if (object == CRATE)  object = -1; break;
		case SDLK_p:		if (object == WODOX)  object = -1; break;
		case SDLK_j:		if (object == SMALL)  object = -1; break;
		case SDLK_l:		if (object == TUBE)   object = -1; break;
		case SDLK_u:		if (object == BELTLF) object = -1; break;
		case SDLK_o:		if (object == BELTBK) object = -1; break;
		case SDLK_i:		if (object == BELTRT) object = -1; break;
		case SDLK_y:		if (object == BELTFT) object = -1; break;
		case SDLK_m:		if (object == MOVING) object = -1; break;
		case SDLK_b:		if (object == BUTTON) object = -1; break;
		case SDLK_s:		if (object == SWITCH) object = -1; break;
		case SDLK_e:		if (object == WARP)   object = -1; break;
		case SDLK_x:		if (object == EMPTY)  object = -1; break;
		default: break;
		}
	      break;
	    ...
	    case SDL_KEYDOWN:
		  ...
		  switch (event.key.keysym.sym)
		    {
		    case SDLK_UP:	shift_lf (); break;
		    case SDLK_DOWN:	shift_rt (); break;
		    case SDLK_RIGHT:	shift_bk (); break;
		    case SDLK_LEFT:	shift_ft (); break;
		    case SDLK_PAGEDOWN:	shift_dn (); break;
		    case SDLK_PAGEUP:	shift_up (); break;
		    default: break;
		    }
		  break;
		}
		  ...
		  switch (event.key.keysym.sym)
		    {
		    case SDLK_RIGHT:	rotate_rt (); break;
		    case SDLK_LEFT:	rotate_lf (); break;
		    default: break;
		    }
		  break;
		}
	      switch (event.key.keysym.sym)
		{
		case SDLK_PAGEDOWN:	if (Y (cursor) > MIN) cursor -= SIZE_2; break;
		case SDLK_PAGEUP:	if (Y (cursor) < MAX) cursor += SIZE_2; break;
		case SDLK_UP:		if (X (cursor) > MIN) cursor -= SIZE; break;
		case SDLK_DOWN:		if (X (cursor) < MAX) cursor += SIZE; break;
		case SDLK_RIGHT:	if (Z (cursor) > MIN) cursor -= 1; break;
		case SDLK_LEFT:		if (Z (cursor) < MAX) cursor += 1; break;
		case SDLK_g:		object = GROUND; break;
		case SDLK_c:		object = CRATE;  break;
		case SDLK_p:		object = WODOX;  break;
		case SDLK_j:		object = SMALL;  break;
		case SDLK_l:		object = TUBE;   break;
		case SDLK_u:		object = BELTLF; break;
		case SDLK_o:		object = BELTBK; break;
		case SDLK_i:		object = BELTRT; break;
		case SDLK_y:		object = BELTFT; break;
		case SDLK_m:		object = MOVING; break;
		case SDLK_b:		object = BUTTON; break;
		case SDLK_s:		object = SWITCH; break;
		case SDLK_e:		object = WARP;   break;
		case SDLK_x:		object = EMPTY;  break;
		case SDLK_w:		if (properties (1 + strrchr (path, '/'))) { save (path); } break;
		case SDLK_r:		load (path); break;
		case SDLK_t:		save ("level.tmp"); play ("level.tmp", "Unnamed level"); break;
		case SDLK_ESCAPE:	keep_going = 0; break;
		case SDLK_F1:	
		  switch (help (str_howtoedit, str_menuedit, "\0twrq"))
		    {
		    case 1:		save ("level.tmp"); play ("level.tmp", "Unnamed level"); break;
		    case 2:		if (properties (1 + strrchr (path, '/'))) { save (path); } break;
		    case 3:		load (path); break;
		    case 4:		keep_going = 0;
		    }
		  break;
		case SDLK_RETURN:	edit_circuit (cursor);
		default: break;
		}
	      break;
      ...
      while (SDL_PollEvent (&event) != 0)
	{
	  switch (event.type)
		...
		case SDLK_F1:
		  ...
		  while (page) switch (page)
		    {
		    case 1: page = help (str_howtolink, str_menulink, "\01234"); break;
		    case 2: page = help (str_howtolink2, str_menulink, "\01234"); break;
		    case 3: page = help (str_howtolink3, str_menulink, "\01234"); break;
		    case 4: page = help (str_howtolink4, str_menulink, "\01234"); break;
		    }
		  break;

		default:		
		  if (isprint (event.key.keysym.unicode) && buf_ptr != buf + sizeof (buf))
		    {
  ...
  for (i = 0; i < SIZE; ++i)
    for (j = 0; j < SIZE; ++j)
	  ...
	  case SWITCH:	     
	    ...
	    if ((surface = TTF_RenderUTF8_Blended (font_normal, lil_buf, color_white)))
	      {
	        dst.x = SCREENX (SPS * j, SPS * i, SPS * k) - surface->w / 2 + 30;
		dst.y = SCREENY (SPS * j, SPS * i, SPS * k) - surface->h / 2 + 10;
		SDL_BlitSurface (surface, NULL, canvas, &dst);
		SDL_FreeSurface (surface);
	      }
  ...
  if ((f = fopen (path, "rb")))
    {
	  ...
	  C_MAP[n].tree = (Uint16 *)malloc(sizeof (Uint16) * C_MAP[n].size);
...
shift_circuit_bk (struct circuit n)
{
  ...
  if (n.tree)
    for (it = n.tree; it < n.tree + n.size; ++it)
      if (*it < NONE)
        {
          *it = SIZE * (*it / SIZE) + (((*it % SIZE) + SIZE - 1) % SIZE);
        }
  return n;
}
...
shift_circuit_rt (struct circuit n)
{
  ...
  if (n.tree)
    for (it = n.tree; it < n.tree + n.size; ++it)
      if (*it < NONE)
        {
          *it = SIZE_2 * (*it / SIZE_2) + (((*it % SIZE_2) + SIZE) % SIZE_2);
        }
  return n;
}
...
shift_circuit_lf (struct circuit n)
{
  ...
  if (n.tree)
    for (it = n.tree; it < n.tree + n.size; ++it)
      if (*it < NONE)
        {
          *it = SIZE_2 * (*it / SIZE_2) + (((*it % SIZE_2) + SIZE_2 - SIZE) % SIZE_2);
        }
  return n;
}
...
rotate_object_lf (Uint8 p)
{
  switch (p)
    {
    case BELTLF: return BELTFT;
    case BELTBK: return BELTLF;
    case BELTRT: return BELTBK;
    case BELTFT: return BELTRT;
    }
  return p;
}
...
rotate_object_rt (Uint8 p)
{
  switch (p)
    {
    case BELTLF: return BELTBK;
    case BELTBK: return BELTRT;
    case BELTRT: return BELTFT;
    case BELTFT: return BELTLF;
    }
  return p;
}
...
consume (char ** buf)
{
  ...
  for (;;)
    switch (**buf)
      {
      case '(': ++*buf; token = TOKEN_LPAREN; return;
      case ')': ++*buf; token = TOKEN_RPAREN; return;
      case '0' ... '9':
        token = 0;
        do 
          {
	...
	while (**buf >= 'a' && **buf <= 'z');
	tok[i] = 0;
	token = strcmp (tok, "or") == 0 ? TOKEN_OR
